...
test("should filter by text value", () => {
  ...
  const selectedExpenses = selectExpense(expenses, filters);
...
test("should filter by startDate", () => {
  ...
  const selectedExpenses = selectExpense(expenses, filters);
  expect(selectedExpenses).toEqual([expenses[2], expenses[0]]);
});
...
test("should sort by date", () => {
  ...
  const selectedExpenses = selectExpense(expenses, filters);
  expect(selectedExpenses).toEqual([expenses[2], expenses[0], expenses[1]]);
});
